...
# Supprimer tout vieux script socket.io residuel au cas ou (pour un fresh start a la fin du fichier)
content = re.sub(r'<script src="https://cdnjs\.cloudflare\.com/ajax/libs/socket\.io/.*?(</body>|</html>)', r'\1', content, flags=re.DOTALL)
content = content.replace('</body>', '').replace('</html>', '').strip()

# Le bloc Socket.IO avec toasts et bip
final_script = """
